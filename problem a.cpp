...
string subtract(string a, string b) //column subtraction
{
    ...
    else if(B>A)                     //if the number two is bigger than the number one we subtract string a from string b
    {
        ...
        result="0";
    for(int i=0; i<=result.length();)   //we erase the zeros if there are any from the beggining of the string
    {
...
string multiply(string a, string b)  //column multiplication
{
    if(a[0]=='0'||b[0]=='0')                        //if any of the numbers is equal to 0 the result is 0
    {
        return "0";
    }
    int n1=(a.length()), n2=(b.length()), n=(n1+n2);   //we find the length of both strings we calculate the sum of their length - 'n'
    int r[n];                                       //we create an array of the size of 'n'
    for (int x=0; x<=n; x++)                        //we fill the array with zeros
    {
        r[x]=0;
    }
    string result(n,'0');                            //we create a string 'result' of the size of 'n' filled with zeros
    for(int i=0;i<n1;++i)                                //the loop to successively multiply the digits
    {
        for(int j=0;j<n2;++j)
        {
            int ca=a[i]-48;                          //we calculate the values of the digits by subtracting 48 to get the integer
            int cb=b[j]-48;
            r[i+j+1]+=(ca*cb);                       //we fill the array with the product of ca*cb
        }
    }
    for(int i=n-1;i>0;--i)                           //wee fill the result string
    {
        if(r[i]>9)                                   //if the element of the array is bigger than 9 we divide this element by 10 and add it to the previous element
            r[i-1]+=r[i]/10;

        result[i] =(r[i]%10)+48;
    }
    result[0]=r[0]+48;
    if(result[0]=='0')                               //if there is a zero in the 0th index of the result string we subtract the result without it
    {
